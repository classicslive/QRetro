...
public:
  void setAccelXHasBeenRead(bool v)
  {
    if (m_AccelAxisWidget[0])
      m_AccelAxisWidget[0]->setEnabled(v);
  }
  void setAccelYHasBeenRead(bool v)
  {
    if (m_AccelAxisWidget[1])
      m_AccelAxisWidget[1]->setEnabled(v);
  }
  void setAccelZHasBeenRead(bool v)
  {
    if (m_AccelAxisWidget[2])
      m_AccelAxisWidget[2]->setEnabled(v);
  }
  void setGyroXHasBeenRead(bool v)
  {
    if (m_GyroAxisWidget[0])
      m_GyroAxisWidget[0]->setEnabled(v);
  }
  void setGyroYHasBeenRead(bool v)
  {
    if (m_GyroAxisWidget[1])
      m_GyroAxisWidget[1]->setEnabled(v);
  }
  void setGyroZHasBeenRead(bool v)
  {
    if (m_GyroAxisWidget[2])
      m_GyroAxisWidget[2]->setEnabled(v);
  }
  void setIllumHasBeenRead(bool v)
  {
    if (m_IllumValueWidget)
      m_IllumValueWidget->setEnabled(v);
  }
...
signals:
  ...
  void spoofLocationChanged(bool enabled, double lat, double lon, double hAcc, double vAcc);
