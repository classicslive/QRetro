...
static_assert(RETRO_MICROPHONE_INTERFACE_VERSION == 1, "Microphone API version updated!");
...
public:
  ...
  bool getParams(const retro_microphone_t *microphone, retro_microphone_params_t *params);

  bool setState(retro_microphone_t *microphone, bool state);

  bool getState(const retro_microphone_t *microphone);

  int read(retro_microphone_t *microphone, int16_t *samples, size_t num_samples);
