...
#define CONNECT_DIGITAL(gp, jp, sig, id)                                                           \
  connect((gp), &QGamepad::sig, this, [jp](bool pressed) { (jp)->setDigitalButton(id, pressed); })
...
#define CONNECT_TRIGGER(gp, jp, sig, id)                                                           \
  connect(                                                                                         \
    (gp), &QGamepad::sig, this, [jp](double v) { (jp)->setAnalogButton(id, qt2lr_analog(v)); })
  ...
  : QRetroInputBackend(parent)
{
  ...
  connect(
    mgr, &QGamepadManager::gamepadConnected, this, &QRetroInputBackendQGamepad::onGamepadConnected);
  connect(mgr, &QGamepadManager::gamepadDisconnected, this,
    &QRetroInputBackendQGamepad::onGamepadDisconnected);
}
...
void QRetroInputBackendQGamepad::connectPort(unsigned port, int deviceId)
{
  ...
  CONNECT_STICK(
    gp, jp, axisRightXChanged, RETRO_DEVICE_INDEX_ANALOG_RIGHT, RETRO_DEVICE_ID_ANALOG_X);
  CONNECT_STICK(
    gp, jp, axisRightYChanged, RETRO_DEVICE_INDEX_ANALOG_RIGHT, RETRO_DEVICE_ID_ANALOG_Y);
